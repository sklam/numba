...
class BaseNativeLowering(abc.ABC, LoweringPass):
    ...
    def run_pass(self, state):
        ...
        with fallback_context(state, msg):
            ...
            # If the code-hash^typemap-hash changed the uid, register a
            # canonical-name alias so that recursive callers (which emit
            # unresolved refs using the pre-mix name) can still be resolved by
            # the runtime linker.
