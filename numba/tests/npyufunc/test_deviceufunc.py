...
class TestUFuncEngine(TestCase):
    ...
    def test_scalar_output_implicit_output(self):
        def check(types, signature, args, kwargs):
            ufe = UFuncEngine(types=types, signature=signature)

            ufe.schedule(args, kwargs)

            self.assertIs(args[0], ufe.inputs[0])
            self.assertIs(args[1], ufe.inputs[1])
            ...
            np.testing.assert_equal(ufe.kernel_args[0],
                                    args[0].reshape(ufe.broadcasted_shapes[0]))
            np.testing.assert_equal(ufe.kernel_args[1],
                                    args[1].reshape(ufe.broadcasted_shapes[1]))

        signature = "(m, n),(p)->()"
        arr0 = np.arange(10).reshape(2, 5)
        arr1 = np.arange(10)

        check(types=(np.float64, np.float32, np.intp), signature=signature,
              args=(arr0, arr1), kwargs={})
